...
while True:
    if counter == 4:
        ...
        for x in range(0, 4):
            cv2.circle(kitap, (int(circles[x][0]), int(circles[x][1])), 3, (0, 0, 255), cv2.FILLED)
        ...
        if key == ord('q'):
            cv2.destroyAllWindows()
            break
#koda tekrar gelirsen eğer resimden kesilen parcanın boyutunu guzel bir sekilde ayarla
